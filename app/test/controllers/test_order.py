...
def __create_sizes_ingredients_beverages(ingredients: list, beverages: list, sizes: list):
...
def test_create(app, ingredients, beverages, size, client_data):
    created_size, created_ingredients, created_beverages = __create_sizes_ingredients_beverages(ingredients, beverages, [size])
    order = __order(created_ingredients, created_beverages, created_size.pop(), client_data)
    ...
    for param, value in order.items():
        ...
        ingredients_in_detail = set(item['ingredient']['_id'] for item in created_order['detail'] if item['ingredient'] != None)
        pytest.assume(not ingredients_in_detail.difference(ingredient_ids))

        beverages_in_detail = set(item['beverage']['_id'] for item in created_order['detail'] if item['beverage'] != None)
...
def test_calculate_order_price(app, ingredients, beverages, size, client_data):
    ...
    pytest.assume(created_order['total_price'] == round(created_size.pop()['price'] + sum(ingredient['price'] for ingredient in created_ingredients) + sum(beverages['price'] for beverages in created_beverages), 2))
...
def test_get_all(app, ingredients, beverages, sizes, client_data):
    created_sizes, created_ingredients, created_beverages = __create_sizes_ingredients_beverages(ingredients, beverages, sizes)
    created_orders = []
    for _ in range(5):
        order = __order(shuffle_list(created_ingredients)[:3], shuffle_list(created_beverages), get_random_choice(created_sizes), client_data)
        created_order, _ = OrderController.create(order)
